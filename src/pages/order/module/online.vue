          ...
          <el-radio v-model="currentScan"
            ...
            <div class="panel-item__bd">
              ...
              <div class="record-data">
                ...
                <div class="record-data__item">左脚<span class="result-text">{{ item.leftFooterResult }}</span></div>
                <div class="record-data__item">右脚<span class="result-text">{{ item.leftFooterResult }}</span></div>
              </div>
            </div>
          </el-radio>
        </div>
            ...
            <div class="sku-item"
              ...
              <div class="sku-value">
                <el-radio-group size="medium"
                                @change="(e) => {handleChangeSpec(e, index)}"
...
<script>
import { getSpu, getListclassiFication, putUpdateNum, getScanResultByPhone } from '@/service/http'
...
export default {
  ...
  methods: {
    handleSubmit () {
      ...
      this.$router.push({ name: 'reserve', query: args })
    },
    ...
    handleChangeScan (val) {
      ...
      this.currentTime = this.scanData.filter(item => item.id === val)[0].scanTime
    },
    handleSearchScan () {
      this.$refs.phoneForm.validate((valid) => {
        if (valid) {
          ...
          getScanResultByPhone(args).then(res => {
            console.log(res)
    ...
    handleChangeSpec (id, index) {
      ...
      if (this.spuData[index].type === 0) {
        // 人群
        this.skuPrice = this.spuData[index].list.filter(item => item.id === id)[0].secondName
      }
    },
    handleScanData (data) {
      let result = []
